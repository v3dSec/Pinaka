...
void decode_djb2(const int *encoded, size_t encoded_len, unsigned char *decoded_shellcode)
{
    ...
    for (int i = 0; i < 256; i++)
    {
        ...
        hash = ((hash << 5) + hash) + (unsigned char)i; // hash * 33 + i
        lookup_table[i] = hash;
    }
...
int main()
{
    ...
    decode_djb2(encoded, shellcode_len, (unsigned char *)shellcode);
